...
-- 우리 학원이 수동 등록한 타 학원과 해당 학원의 수업입니다.
-- SaaS 연결 학원 공유와 달리, 원장이 내부 참고용으로 등록하는 일정입니다.
create table public.external_academies (
  id uuid primary key default gen_random_uuid(),
  academy_id uuid not null references public.academies(id) on delete cascade,
  name text not null,
  category text,
...
create table public.external_academy_classes (
  id uuid primary key default gen_random_uuid(),
  academy_id uuid not null references public.academies(id) on delete cascade,
...
create table public.student_external_class_enrollments (
  id uuid primary key default gen_random_uuid(),
  academy_id uuid not null references public.academies(id) on delete cascade,
  student_id uuid not null references public.students(id) on delete cascade,
...
create unique index student_schedules_one_off_uidx
  ...
  where is_active = true and schedule_date is not null;
create index external_academies_academy_active_idx
  on public.external_academies(academy_id, is_active);
create index external_academy_classes_academy_day_idx
  on public.external_academy_classes(academy_id, day_of_week, start_time)
  where is_active = true;
create index external_academy_classes_external_academy_idx
  on public.external_academy_classes(external_academy_id)
  where is_active = true;
create index student_external_class_enrollments_student_idx
  on public.student_external_class_enrollments(student_id)
  where is_active = true;
create index student_external_class_enrollments_class_idx
  on public.student_external_class_enrollments(external_academy_class_id)
  where is_active = true;
create unique index student_external_class_enrollments_active_uidx
