...
| Spring backend | 운영 API 코드 전환 완료, Production 미연결, 로컬 검증 및 Next.js fallback 유지 |
| Backend env | Production에 `NEXT_PUBLIC_BACKEND_API_URL` 미설정 |

## 3. 권한별 운영 확인

2026-06-26 기준 아래 계정은 Supabase Auth 로그인과 `profiles` 연결을 확인했습니다. 역할별 기본 검증은 아래 3개 계정을 기준으로 합니다.

| 계정 | 확인 |
| --- | --- |
| `owner@test.com / 1234` | owner, active, 더배움프라임영수학원 연결. 홈, 출석, 문자, 관리, 리포트, 이력 접근 확인 대상 |
| `teacher@test.com / 12345678` | teacher, active, 더배움프라임영수학원 연결. 담당 반만 표시, 관리 탭 숨김, 전화번호 숫자 비노출 확인 대상 |
| `assistant@test.com / 12345678` | assistant, active, 더배움프라임영수학원 연결. 연락 기록 저장 가능, 테스트 발송 버튼 권한 안내/비활성 확인 대상 |

보조 확인 계정:

- `other-owner@test.com / 1234`: 다솜수학학원 owner 계정. 학원 간 스케줄 공유 확인용입니다.
- `uat-teacher@test.com`, `uat-assistant@test.com`: 과거 UAT 계정입니다. 현재 역할별 기본 검증 계정으로 사용하지 않습니다.
...
- [operations-real-data-template-2026-06.csv](./operations-real-data-template-2026-06.csv)
...
실제 발송은 T-310에서 별도 승인 후 진행합니다. 상세 기준은 [SOLAPI 테스트 번호 제한 발송 준비](./solapi-test-number-readiness-2026-06.md)를 따릅니다.
