# 현재 로드맵

## 1. 현재 상태

- Next.js + Supabase 기반 MVP 구현 완료
- 200명 파일럿 seed 기준 데이터 정합성 정상화 완료
- 모바일 390px 기준 UAT와 UX 점검 문서화 완료
- 원장/선생님/보조 선생님 계정 기준 권한 검증 진행
- Spring Boot 운영 API 코드 전환 완료, Railway 배포 준비 단계
- 월요일 원장 미팅 이후 제품 중심이 “홈 대시보드/후속 연락”에서 “출석부 중심 운영장부 + 일괄 문자 + 비용 안내”로 이동할 가능성이 확인됨

## 2. 단기 목표

- 출석부 중심 제품 방향을 먼저 정리하고 후속 이슈를 P0/P1/P2로 분리
- P0는 출석부 메뉴 우선순위, 출석/지각/결석 단순화, 다중 선택, 이름 변수 치환 문자
- 6월 중순까지 Spring Boot 전환 코드는 운영 API 기준으로 완료됐지만, Railway 배포보다 고객 미팅 기반 출석부 UX 재정의가 우선
- Next.js 앱은 `apps/frontend`, Spring Boot는 `apps/backend`로 분리 완료
- 200명 seed 검증에서 실제 구조 데이터 20~30명 운영 전환 검증으로 이동
- SOLAPI 테스트 번호 1건 제한 발송 준비
- PC 홈/관리 콘솔은 선생님 PC 사용을 고려한 업무형 레이아웃으로 개선 완료
- 앞으로 출석부/명단/리포트 같은 운영 화면은 PC 1440px 사용성을 1순위로 보고, PC는 왼쪽 사이드바 기반 업무 콘솔, 모바일은 현장 확인용 하단 탭 흐름을 유지
- 출석부는 원장/관리자 기준 `달력 보기 → 날짜 상세 → 학생 스케줄 drawer` 흐름을 기본 운영 콘솔 방향으로 확장
- 선생님/보조 선생님은 출석부 안에서도 `오늘 처리` 중심으로 유지하고, 원장용 월간 달력과 역할별 기본 화면을 분리
- 운영 DB 정리는 seed-only 정리와 전체 운영 데이터 리셋을 분리하고, 실제 데이터 투입 직전에는 `Preview -> Cleanup -> Verify` 순서로 사람이 Supabase SQL Editor에서 실행
- 기존 PR/이슈 정리
- 문서 기준 정리

## 3. 중기 목표

- Next.js 앱을 `apps/frontend`에서 안정화
- Spring Boot API를 `apps/backend`에 추가 완료
- IntelliJ에서 루트 프로젝트 기준으로 관리 가능하게 정리
- Railway 결제 후 Spring Boot backend를 배포하고 Vercel frontend와 제한적으로 연결
- 실제 운영 데이터는 20~30명부터 투입하고, dry-run 검증 후 테스트 번호 1건 발송으로 확대
- 출석부에서 지각/결석/선택 문자/학생 메모/동명이인 구분이 실제 원장 운영 흐름과 맞는지 확인
- 교재비/수강료는 결제 시스템이 아니라 금액 기록과 월말 안내 문자 수준으로만 설계

## 4. 장기 목표

- Spring Boot를 핵심 백엔드로 사용
- Next.js는 화면/UX 중심으로 유지
- Supabase는 PostgreSQL/Auth 기반으로 유지하거나 점진적 대체 검토
- 출석/문자/보강/권한/감사 로그/리포트 도메인을 백엔드 중심으로 정리
- 출석부 중심 운영장부에서 월별 출석, 문자, 교재비 안내 기록을 확인할 수 있게 확장

## 5. 하지 않을 것

- 당장 전체 재작성
- 기존 Next.js API 즉시 제거
- Supabase Auth 즉시 제거
- 운영 화면을 Spring Boot에 직접 종속
- 고객 피드백 없이 대규모 기능 추가
- Railway 결제 전 Production에 Spring backend URL 연결
- 테스트 번호 검증 전 실제 학부모 번호 문자 발송
- 교재비/수강료 요구를 바로 카드 결제, 청구서, 미납 관리, 세금계산서 기능으로 확대
- 전체 학원관리 프로그램과 같은 폭으로 경쟁하기 위한 대규모 기능 추가
