...
| 순서 | 티켓 | 우선순위 | 이유 |
| --- | --- | --- | --- |
| 1 | T-501 실제 구조 데이터 운영 전환 검증 | P0 | 현재 UI는 seed/demo 데이터로 확인됐으므로, 실제 반 1~2개/학생 20~30명 구조에서 출석/문자/관리 흐름을 검증해야 합니다. |
| 2 | T-310 SOLAPI 테스트 번호 제한 발송 | P0 | dry-run만으로는 실제 문자 전환 리스크를 확인할 수 없으므로, 테스트 번호 1건 수신과 `message_logs` 상태를 확인해야 합니다. |
| 3 | T-658 달력 보기용 출석 요약 API 추가 | P1 | 현재 달력 월간 요약은 프론트 조합 중심이라 실제 데이터가 늘면 성능/정합성 리스크가 있습니다. |
| 4 | T-660 날짜별 연락 필요 학생 일괄 문자 연결 | P1 | 달력 날짜 상세의 `연락 필요` 탭을 실제 업무 처리로 연결하면 원장/선생님 사용성이 커집니다. |
| 5 | T-659 학생 스케줄 drawer 데이터 보강 | P1 | drawer가 제품처럼 보이려면 월간 출석/최근 연락/주간 스케줄 데이터 정확도를 더 높여야 합니다. |
| 6 | T-708 보안/개인정보 운영 체크리스트 | P1 | 실제 개인정보와 문자 발송을 다루기 전 외부 설명 가능한 운영 기준이 필요합니다. |
