...
| T-657 출석부 달력 보기 및 날짜 드릴다운 UI 구현 | 진행 중 | P0 | 원장/관리자가 월간 달력에서 날짜를 선택하고 수업/학생/연락 필요 상세와 학생 drawer를 확인 | 기존 출석/학생/연락 데이터 | PR 확인 후 main 반영 |
| T-657 PC 운영 콘솔 Shell 재정의 | 진행 중 | P1 | PC 원장/관리자 화면을 왼쪽 사이드바 기반 업무 콘솔로 고정 | T-655 메뉴 구조 정리 | PR 확인 후 main 반영 |
| T-656 출석부 장부형/테이블형 레이아웃 설계 | 대기 | P0 | PC 출석부를 학생 row 중심 장부형 구조로 설계 | T-657 PC Shell 정리 | T-657 이후 설계/구현 범위 확정 |
