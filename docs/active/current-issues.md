...
| T-628 docs 정리 및 monorepo 전환 준비 | 진행 중 | P1 | 문서 기준 정리와 구조 전환 준비 | 현재 docs 목록 점검 | PR 리뷰 후 병합 |
| T-629 Next.js 앱 frontend 디렉터리 이동 | 후속 후보 | P1 | monorepo 목표 구조로 프론트 이동 | T-628 완료 | 별도 브랜치/PR |
| T-630 Spring Boot backend skeleton 추가 | 후속 후보 | P2 | 백엔드 전환을 위한 최소 Spring Boot 앱 추가 | T-629 또는 경로 정책 확정 | skeleton 범위 확정 |

## 현재 원칙

- GitHub Issue는 실제 실행 카드입니다.
- 문서만으로 끝나는 판단은 `docs/active`에 남기고, 구현이 필요할 때만 GitHub Issue를 만듭니다.
- 6월 중순 MVP 동결 전에는 DB schema 변경과 큰 기능 추가를 피합니다.
