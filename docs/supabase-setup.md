...
학생 목록, 스케줄, 출석부 화면을 실제처럼 테스트하려면 아래 파일을 추가로 실행합니다.

```text
supabase/pilot-volume-seed.sql
```

이 파일은 더미 반 6개, 더미 원생 48명, 주간 스케줄, 오늘 날짜 출석 상태를 넣습니다. 다시 실행해도 같은 더미 학생은 업데이트되고, 볼륨 테스트용 스케줄은 재생성됩니다.
